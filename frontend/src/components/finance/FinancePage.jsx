  ...
  const fetchFinanceData = async () => {
    try {
      ...
      const [paymentsRes, studentsRes] = await Promise.all([
        api.get('/showPayments').catch(() => ({ data: { data: [] } })),
      ...
      const lastMonthPaidCount = lastMonthPayments.filter(p => p.status === 'paid').length;
      
      const paymentGrowth = lastMonthPaidCount > 0 
        ? ((currentMonthPaidCount - lastMonthPaidCount) / lastMonthPaidCount * 100)
        : currentMonthPaidCount > 0 ? 100 : 0;
