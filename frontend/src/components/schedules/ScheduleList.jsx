...
const ScheduleList = () => {
  ...
  const fetchSchedules = async () => {
    try {
      ...
      let response;
      try {
        response = await api.get('/showSchedules');
      } catch (err) {
        console.log('Trying /schedules endpoint...');
      ...
      // Handle different response structures and ensure it's an array
      let schedulesData = response.data.data || response.data || [];
      
      // Ensure schedulesData is always an array
      if (!Array.isArray(schedulesData)) {
        console.warn('Schedules data is not an array:', schedulesData);
        schedulesData = [];
      }
      
      console.log('Schedules array:', schedulesData);
      console.log('First schedule item:', schedulesData[0]); // Debug first item structure
                      ...
                      <div className="flex items-center justify-between mb-4">
                        <div className="flex items-center space-x-3">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-gray-600">Course ID: {schedule.course_id}</p>
                          </div>
                        </div>
